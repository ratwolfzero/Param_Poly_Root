...
# ================================================================
# MULTIPLICITY CLUSTERING (for numerical fallback)
# ================================================================
def cluster_roots_by_proximity(raw_roots, threshold=FALLBACK_CLUSTER_THRESHOLD):
    """
...
# ================================================================
# TRIPLET DELTA CIRCLE INTERSECTION ANALYSIS
# ================================================================
def circle_intersection_info(center1: complex, r1: float, center2: complex, r2: float) -> Dict:
    ...
    if d > sum_r + 1e-12:
        ...
        result['status'] = 'touching externally'
        t = r1 / (r1 + r2)
        p = center1 + t * (center2 - center1)
        result['intersection_points'] = [p]
    elif d < diff_r - 1e-12:
        result['status'] = 'one circle completely inside the other (no intersection)'
    elif abs(d - diff_r) < 1e-12:
        result['status'] = 'touching internally'
        if r1 > r2:
            p = center1 + (r1 / d) * (center2 - center1)
        else:
            p = center2 + (r2 / d) * (center1 - center2)
        result['intersection_points'] = [p]
    else:
        result['status'] = 'intersecting'
        result['overlap'] = sum_r - d
        a = (r1*r1 - r2*r2 + d*d) / (2*d)
        h_sq = r1*r1 - a*a
        if h_sq < 0:
            h_sq = 0.0
        h = np.sqrt(h_sq)
        p_mid = center1 + (a/d) * (center2 - center1)
        perp = 1j * (center2 - center1) / d
        if h > 1e-12:
            p1 = p_mid + h * perp
            p2 = p_mid - h * perp
            result['intersection_points'] = [p1, p2]
        else:
            result['intersection_points'] = [p_mid]
    return result

def print_circle_analysis(triplets: List[Dict]):
    """Pretty print intersection analysis for all circle pairs."""
    n = len(triplets)
    if n < 2:
...
for (root_mpc, mult), (_, _, delta, _) in triplet_cache.items():
    triplets_list.append({
        ...
        'mult': mult,
        'delta': float(abs(delta)) if delta is not None else float('inf')
    })

# Perform intersection analysis
print_circle_analysis(triplets_list)

# Optional: plot only the delta circles with intersections (Figure 3)
def plot_delta_circles(triplets: List[Dict]):
    """Create a dedicated figure showing δ circles and their intersections."""
    if not triplets:
        return
    finite_triplets = [t for t in triplets if np.isfinite(t['delta'])]
    ...
    for t in finite_triplets:
        ...
        mult = t['mult']
        delta = t['delta']
        circle = Circle((root.real, root.imag), delta,
                        fill=False, edgecolor=colors[mult], linewidth=2,
                        linestyle='--', alpha=0.7)
    ...
    for i in range(n):
        t1 = finite_triplets[i]
        r1 = t1['delta']
        for j in range(i+1, n):
            t2 = finite_triplets[j]
            r2 = t2['delta']
            info = circle_intersection_info(t1['root'], r1, t2['root'], r2)
